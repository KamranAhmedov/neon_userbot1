...
async def audio(e):
  ...
  ad = e.pattern_match.group(1)
  hm = YouTube(f"{ad}")
  videoad = hm.title
  await e.edit(f"`{videoad}` **yüklənir...**")
  Neon = YouTube(f"{ad}").streams.filter(file_extension='mp4').first().download()
  await e.edit(f"`{videoad}` **mp3 olaraq hazırlanır...**")
  name = hm.title + ".mp3"
  my_clip = mp.VideoFileClip(Neon)
  my_clip.audio.write_audiofile(name)
  await e.edit("**Fayl göndərilir...**")
...
CmdHelp('song').add_command(
    'music', '«YouTube link/URL»', 'YouTube-dan Musiqini MP3 olaraq endirər.'
).add_command(
